...
**엔터티 헤더**
- 18자에 불과한(Content-Length: 18) 플레인 텍스트 문서(Content-Type: text/plain)를 의미합니다.
- 빈 줄(CRLF)은 헤더 필드와 본문의 시작을 나눕니다.
...
엔터티의 비트 집합을 텍스트 파일의 글자들로 변환하기 위한 `charset` 매개변수가 대표적입니다.
...
- MIME "멀티파트" 이메일 메시지는 서로 붙어있는 여러 개 메시지를 포함하며, 하나의 복합 메시지로 보내집니다.
- 각 구성 요소는 자족적으로 자신에 관해 서술하는 헤더를 포함합니다. 여러 구성요소가 이어져 있고 문자열 하나로 서로의 경계가 식별됩니다.
- HTTP는 멀티파트 본문도 지원합니다. 그러나 일반적으로 폼을 채워 제출할 때와 문서 일부분을 실어 나르는 범위 응답을 할때 두 가지 경우에만 사용됩니다.

### 15.4.3 멀티파트 폼 제출

- HTTP 폼을 채워서 제출하면, 가변 길이 텍스트 필드와 업로드 될 객체는 각각이 멀티파트 본문을 구성하는 하나의 파트가 되어 보내집니다.
- 멀티 파트 본문은 다른 종류와 길이의 값으로 채워진 폼을 허용합니다.
...
1. 웹 서버가 원본 Content-Type 과 Content-Length 헤더를 수반한 원본 응답 메시지를 생성합니다.
2. 콘텐츠 인코딩 서버가 인코딩된 메시지를 생성합니다. 이때 인코딩된 메시지는 Content-Type 은 같지만 Content-Length 는 다릅니다. 이후 Content-Encoding 헤더를 인코딩된 메시지에 추가하여, 애플리케이션이 디코딩할 수 있도록 합니다.
...
클라이언트는 `자신이 지원하는 인코딩의 목록을 Accept-Encoding 요청 헤더를 통해 전달`합니다. HTTP 요청에 Accept-Encoding 헤더를 포함하지 않는다면, 서버는 클라이언트가 어떤 인코딩이든 받아들일 수 있다고 간주합니다. (Accept-Encoding: * 과 같아요)
...
### 1. Content-Type mismatch로 인해 장애를 겪은적이 있으신가요?
- ex) 백엔드가 JSON인데 Content-Type을 text/plain으로 보내서 파싱이 에러났다거나...

### 2. 다른 인코딩 방식의 적용으로 인해 체감 속도 차이를 경험해본 적이 있으신가요?
...
- 이 중 범위 요청과 델타 인코딩이 대표적인 두 가지입니다.
- 이 둘 다, 클라이언트가 자신이 가진 리소스의 사본이 서버가 가진 것과 정확히 같은지 판단하고 상황에 따라 새 인스턴스를 요청할 수 있는 능력을 갖출 것을 요구합니다.

## 15.8 검사기와 신선도

**조건부 요청**

클라이언트가 서버에게 갖고 있는 버전을 말해주고 검사기를 사용해 자신의 사본 버전이 더 이상 유효하지 않을 때만 사본을 보내달라고 요청하는 것입니다.

세 가지 주요 개념은 다음과 같습니다. 신선도, 검사기, 조건
...
### 1. 저는 프론트 개발자인지라 Cache-Control이라는 헤더를 백엔드에서 많이 활용하는 편인가요?

### 2. 여러분의 서비스라면 “델타 인코딩 vs 전체 다시 전송” 중 어떤 전략을 택하실건가요?
